...
<html>
	<head>
        ...
        $(function(){/*문서가 로드되면 function을 실행하라  */
            $("#pass").blur(function(){/*아이디가 id인것을 찾아 포커즈를 빠져나갈때 발생하는 이벤트  */                
                if($(this).val() == ""){
                    $("#pass_check_msg").html("패스워드를 입력하세요.").css("color","red").attr("data-check","0");//선택자를 .연사자추가해서 계속 사용가능  
                    $(this).focus();
                } else {
                    checkPassAjax();				
                }   
        ...
        /* 아이디 중복 체크(비동기통신) */
        function checkPassAjax(){//id값을 post로 전송해서 서버와 통신하여 중복 결과 json 형태로 받아오는 함수
            var id = $("#pass").val();
            $.ajax({				//비동기통신방법, 객체로 보낼때{}사용
                url : "./pass_check.php",
                type : "POST",
                ...
                success : function(data){
                    if(data.check){			//json사용했기때문에 data.으로 접근가능
                        $("#pass_check_msg").html("사용 가능한 패스워드입니다.").css("color", "blue").attr("data-check","1");
                    } else {
                        $("#pass_check_msg").html("중복된 패스워드입니다.").css("color", "red").attr("data-check","0");
                        $("#pass").focus();
                    }
                }
            });            
